...
class Mask(QtWidgets.QWidget):
  def __init__(self, ser, fps, imgWidth, imgHeight, infile, outfile, numCaptures, brightness, sleepTime):
    ...
    # Measurement rows
    self.numMeasurements = self.Hr.shape[0] * 3
    self.currCapture = 0
  ...
  def initSelf(self):
    ...
    self.sensor_readings = np.zeros((self.numMeasurements, 3))
  ...
  def updateProjector(self, count, channel=0):
    # Need to scale mask aspect ratio accordingly.
    # Measurement "row"
    mask1 = np.reshape(self.Hr[count, :], (self.imgHeight, self.imgWidth))
    mask = np.zeros((mask1.shape[0], mask1.shape[1], 3))
    mask[:,:,channel] = mask1
    mask = np.require(mask, np.uint8, 'C')
    QI = QtGui.QImage(mask, mask.shape[1], mask.shape[0], QtGui.QImage.Format_RGB888)
    ...
    self.label.setPixmap(QtGui.QPixmap.fromImage(QI).scaled(int(scaledHeight * scaledWidthFactor), int(scaledHeight)))
  ...
  def updateData(self):
    ...
    if self.count < self.numMeasurements + shiftCorrection:
      ...
      if (self.count < self.numMeasurements):
        self.updateProjector(cnt//3, channel=cnt%3)
      ...
      if (self.count < shiftCorrection):
        ...
        if (PRINT_DELAY_STATEMENT and self.count > shiftCorrection and self.count < shiftCorrection + 5):
          print("Time delta between captures in s: %s" % timeDelta)
        elif (self.count > shiftCorrection):
          if (PRINT_DELAY_STATEMENT and timeDelta > self.sleepTime * 1.35 or timeDelta < self.sleepTime * 0.65):
            print("Time delta between captures in s: %s" % timeDelta)
        ...
        self.sensor_readings[self.dataLoc//3, self.dataLoc%3] = data
      ...
      # N = width
      reshapeHeight = int(np.ceil(len(self.sensor_readings[:,0]) / self.imgWidth))
      reshapeSize = reshapeHeight * self.imgWidth

      # Awkwardly fill in to get a full row for displaying (why ceil is used)
      padAmount = reshapeSize - len(self.sensor_readings[:,0])

      # Should be column vector!
      paddedSensorReadings_red = np.append(self.sensor_readings[:,0], np.transpose([np.amax(self.sensor_readings[:,0])] * padAmount), axis = 0)
      paddedSensorReadings_green = np.append(self.sensor_readings[:,1], np.transpose([np.amax(self.sensor_readings[:,1])] * padAmount), axis = 0)
      paddedSensorReadings_blue = np.append(self.sensor_readings[:,2], np.transpose([np.amax(self.sensor_readings[:,2])] * padAmount), axis = 0)
      paddedSensorReadings = np.zeros((len(paddedSensorReadings_red), 3))
      paddedSensorReadings[:,0] = paddedSensorReadings_red
      paddedSensorReadings[:,1] = paddedSensorReadings_green
      paddedSensorReadings[:,2] = paddedSensorReadings_blue

      if (reshapeHeight != int(len(self.sensor_readings[:,0]) / self.imgWidth)):
      ...
      np.save("%s_red.npy" % saveFile, self.sensor_readings[:,0])
      np.save("%s_green.npy" % saveFile, self.sensor_readings[:,1])
      np.save("%s_blue.npy" % saveFile, self.sensor_readings[:,2])
      self.currCapture += 1

      #plt.figure()
      #img = np.reshape(paddedSensorReadings[:,0], (reshapeHeight, self.imgWidth))
      #plt.imshow(img, cmap='gray')
      #plt.title('Capture Image Results')
      #plt.show()

      print("Min sensor reading: %s" % np.amin(paddedSensorReadings[:,0]))
      print("Max sensor reading: %s" % np.amax(paddedSensorReadings[:,0]))
  ...
  def rescale(self):
    ...
    self.label.setPixmap(QtGui.QPixmap.fromImage(QI).scaled(int(self.dispWidth), int(self.dispHeight), QtCore.Qt.KeepAspectRatio))
